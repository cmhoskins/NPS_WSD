...
Libraries:
```{r}
# install and load packages using the setup script
source("setup.R")
```
...
Create objects for select park, aoi(park with buffer), buildings (with buffer), and select buildings:
```{r}
park <- get_select_parks("SLBE")

aoi <- get_aoi(park, 1000)

buildings <- get_buildings(park, 1000)
 
#select_buildings <- get_select_buildings(buildings, "dundo", BLDGNAME)
```
...
b <- a + 
  mapview(mi_select_wells,
          col.regions = "dodgerblue",
           cex = 5,
          layer.name = 'USGWD') +
  mapview(mi_select_wells_state,
          col.regions = "purple",
          cex = 5,
...
# map of water sources by region
c <-mapview(select_source_region,
                 col.regions = "hotpink",
                 layer.name = "Water Sources")

# map of water sources by state
d <- mapview(select_source_state,
             col.regions = "hotpink",
             cex = 9,
             layer.name = "Water Sources")

# get parks for a state
parks_state <- getParkSystem() %>%
  filter(STATE == "WV")

# map of state water sources and parks
e <- d + mapview(parks_state) 

# map of state usgwd wells and database state water sources
f <- mapview(vt_select_wells) + mapview(select_source_state,
                 col.regions = "hotpink",
                 layer.name = "Water Sources")
...
View and Tidy WSD
```{r}
# options for looking at the data by region
## glimpse()
